...
- `fix/comaprison-issue`: Shortcircuit comparisons
  - Eg. `a == b && c > 2` should jump to skip the second check if first is false
...
- Function variable scope does not get dropped after an error is raised inside a function body
  ...
  - Results in "`b` is not defined" *and* "`x` is already defined"
