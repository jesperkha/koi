use koi_ast::{Node, NodeId, Pos, TokenKind};

use crate::types::{CastKind, NO_TYPE, TypeId};
...
impl_node_for_enum!(Stmt {
    meta => [
        Return, VarDecl, VarAssign, If, While, For, Break, Continue, OpAssign,
    ],
    delegate => [ExprStmt]
});

impl_node_for_enum!(Expr {
    meta => [Literal, Call, Member, NamespaceMember, Binary, Unary, Cast],
    delegate => []
});
...
impl_typed_node_enum!(Stmt {
    Return, VarDecl, VarAssign, ExprStmt, If, While, Break, Continue, For, OpAssign,
});

impl_typed_node_enum!(Expr {
    Call, Literal, Member, NamespaceMember, Unary, Binary, Cast,
});
