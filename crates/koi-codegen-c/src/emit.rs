...
use crate::ast::{Ast, BinaryOp, Decl, Expr, Stmt, Type, UnaryOp};
use koi_common::config::{Config, PathManager};
use koi_ir::{self as ir, ConstId, IRTypeId, IRTypeInterner, ParamId, Unit};
...
pub fn emit(unit: Unit, _config: &Config, pm: &PathManager) -> Ast {
    ...
    for decl in unit.decls {
        let decl = match decl {
            ir::Decl::Extern(ext) => Decl::ExternFunc {
                name: ext.name,
                params: ext
                    .params
                    .iter()
            ...
            ir::Decl::Func(func) => FuncEmitter::new(func, &unit.types, &unit.data).emit(),
        };
